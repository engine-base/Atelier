...
/** relay 未接続時の接続フロー (Bridge 起動手順 + コマンドコピー)。 */
function ConnectFlow() {
  const [copied, setCopied] = React.useState(false);
  const command = `ATELIER_API_URL=${api.API_BASE} ATELIER_BRIDGE_TOKEN=<管理者から発行されたトークン> node dist/headless.js --loop`;
  const copy = () => {
  ...
  return (
    <div className="rounded-md border border-border bg-surface-variant/50 p-3">
      ...
      <ol className="mt-1.5 list-decimal space-y-1 pl-4 text-[11.5px] leading-[1.6] text-on-surface-variant">
        <li>ご自分の PC で Claude (デスクトップ / CLI) にログインしておく</li>
        <li>Atelier Bridge アプリを起動する (または下のコマンドで headless 起動)</li>
        <li>このパネルの表示が「Bridge 接続中」になれば完了</li>
      </ol>
      ...
      <p className="mt-1.5 text-[10.5px] leading-[1.5] text-on-surface-variant">
        Bridge トークンはワークスペース管理者が発行します。あなたの Claude
        認証情報がこの PC の外へ送られることはありません。
      </p>
    </div>
  );
}
