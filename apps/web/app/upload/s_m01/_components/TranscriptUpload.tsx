...
export function TranscriptUpload({ onUpload }: TranscriptUploadProps) {
  ...
  return (
    <section aria-label="議事録アップロード" className="flex flex-col gap-md">
      <h1 className="text-headline-md font-bold text-on-surface">議事録アップロード</h1>
      <label className="flex flex-col gap-sm">
        <span className="text-label-lg font-semibold text-on-surface">音声 / 動画ファイル</span>
        <input
          type="file"
          accept="audio/*,video/*"
          onChange={onChange}
      ...
      {status === 'uploading' ? (
        <p role="status" aria-live="polite" className="text-label-md text-on-surface-variant">
          アップロード中…
        </p>
      ) : null}
      {status === 'error' && error ? (
      ...
      {status === 'done' ? (
        <article aria-label="文字起こし結果">
          <h2 className="text-label-lg font-semibold text-on-surface">文字起こし</h2>
