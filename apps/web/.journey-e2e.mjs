...
await row('J-06', owner, async () => {
  ...
  await owner.getByLabel(/案件/).first().fill(`LP一式お見積 ${mark}`);
  await owner.getByLabel(/商談概要/).fill('LP 制作一式 45 万円 (ヒーロー/実績/問い合わせ、4 週間)');
  await owner.getByRole('button', { name: 'ドラフト生成' }).click();
  await owner.waitForTimeout(2500);
  const did = sql(`select id from workflow_outputs where project_id='${S.pid}' and stage in ('estimate','proposal') and deleted_at is null limit 1`);
  expect(did, 'sales_docs 行が無い');
  const v1 = sql(`select version from workflow_outputs where id='${did}'`);
  // 本文の手直し: 一覧から選択 → 編集 → 保存 (PATCH: in-place 編集)
  await owner.getByText(`LP一式お見積 ${mark}`).first().click();
  await owner.getByRole('button', { name: '編集', exact: true }).click();
  await owner.waitForTimeout(500);
  // プレビュー側の編集 textarea は最後 (最初は左の商談概要フォーム)
  const ta2 = owner.locator('textarea:visible').last();
  await ta2.fill('LP 制作一式 45 万円 (改訂: 実績セクションを事例 3 件に拡充)');
  await owner.getByRole('button', { name: /^保存/ }).click();
  await owner.waitForTimeout(2000);
  expect(sql(`select summary from workflow_outputs where id='${did}'`).includes('事例 3 件'), 'PATCH 編集が DB に反映されない');
  // 改訂版: 同種の見積をもう一度生成 → 版数が +1 される (版数=生成回数の製品仕様)
  await owner.getByLabel(/顧客名/).fill('小松商事');
  await owner.getByLabel(/案件/).first().fill(`LP一式お見積 改訂 ${mark}`);
  await owner.getByLabel(/商談概要/).fill('改訂版: 実績セクションを事例 3 件に拡充した見積');
  await owner.getByRole('button', { name: 'ドラフト生成' }).click();
  await owner.waitForTimeout(2500);
  const v2 = sql(`select max(version) from workflow_outputs where project_id='${S.pid}' and stage='estimate' and deleted_at is null`);
  expect(Number(v2) > Number(v1), `version ${v1}→${v2}`);
  return `sales_doc=${did.slice(0, 8)} 編集反映 OK / 版数 ${v1}→${v2} (再生成)`;
});
