...
| PS-28 | 本番 API 並行 | owner JWT で GET /me を同時 10 本 × 3 回、GET /projects を直列 20 本 | 5xx が 0 (画面 1 枚の初期ロードは 4〜6 本を同時に発行する) | **FAIL (2026-09-03)**: 同時 30 本で 500 が 2 本、直列 20 本で 500 が 3 本 (1〜2 秒で失敗 = プール待ちの 10 秒ではない)。参照 ID 304f0a0e-… ほか。**原因確定 (2026-09-03 error_log)**: Session pooler の上限 15 本を API 2 台 × 30 本が超過 (EMAXCONNSESSION) = GAP-291 → **PASS prod(pre-launch) (2026-09-03 07:09 再測)**: switch-db-pooler.yml で transaction pooler へ切替後、同時 10 本 × 3 回 = 30 本 全 200 / 直列 20 本 全 200 / 画面相当 6 本同時 × 5 回 全 200、5xx = 0。証拠 evidence/ps28-30-prod-20260903.txt | T-I-24 | L4 |
| PS-30 | DB 接続の上限 (G-11) | API が使う接続文字列の種類 (直結 / Session pooler / Transaction pooler) と、その上限本数を実環境で確認し、`ATELIER_DB_POOL_SIZE + MAX_OVERFLOW × 台数` が上限を超えていないことを見る | API の要求本数 ≤ 接続先の上限。Session pooler (15 本) を複数台で共有しない。transaction pooler なら prepared statement 無効 (自動) と押し出し通知の直結 (`ATELIER_DB_LISTEN_URL`) が設定済み | **PASS prod(pre-launch) (2026-09-03)**: switch-db-pooler.yml 実行 (run 33726180580) で `ATELIER_DB_URL`=transaction pooler (6543) / `ATELIER_DB_LISTEN_URL`=直結 を設定、スモーク 200。切替前は Session pooler 15 本に対し 2 台 × 30 本で超過していた (error_log 20 件全部 EMAXCONNSESSION)。**残り**: 押し出し通知 (LISTEN) が直結で張れているかは運営画面 S-T01 の行でしか見えない (運営権限が要る → BLOCKED 継続。張れていなくても自動でポーリングへ戻る設計 GAP-202) | T-I-24 | L4 |
| PS-31 | web デプロイ鮮度 (commit) | `curl -s https://atelier-web-coral.vercel.app/signin` の出力から `name="atelier-build" content="..."` を読む (表の中でパイプ記号を書くと列がずれるので grep へ繋がない書き方にする) で本番 web の commit を読み、main HEAD と比べる | meta atelier-build の値が main HEAD の先頭 12 桁と一致 (Vercel が最新 main を配信している)。一致しなければ Vercel のデプロイが止まっている/失敗している | **FAIL (2026-09-03 09:59)**: deploy #83 (main e1a13ba) 後も本番 /signin に atelier-build 無し・`age: 19784` (5.5 時間前のキャッシュ)。手元の `next build` で原因確定 = /t-uc-35 の useSearchParams に Suspense 境界が無くプリレンダー失敗 (GAP-316)。修正後の main 反映で再測 → **PASS (2026-09-03 10:20)**: main 833fee5205f2 / 本番 meta atelier-build=833fee5205f2 で一致 (Vercel 自動ビルド復旧) | T-F-06 | L4 |
| PS-29 | 本番 公開 API | 未認証で GET /public/legal-documents?locale=ja と 3 種の詳細 | 全部 200 | **PASS prod(pre-launch) (2026-09-03 07:09 再測)**: 一覧 200 (21,622 bytes) / terms 200 / privacy 200 / tokushoho 200。原因は GAP-291 (Session pooler 上限) と同一で、transaction pooler 切替後に解消。証拠 evidence/ps28-30-prod-20260903.txt | T-I-24 | L4 |
