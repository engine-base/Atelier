...
| PS-28 | 本番 API 並行 | owner JWT で GET /me を同時 10 本 × 3 回、GET /projects を直列 20 本 | 5xx が 0 (画面 1 枚の初期ロードは 4〜6 本を同時に発行する) | **FAIL (2026-09-03)**: 同時 30 本で 500 が 2 本、直列 20 本で 500 が 3 本 (1〜2 秒で失敗 = プール待ちの 10 秒ではない)。参照 ID 304f0a0e-… ほか。**原因確定 (2026-09-03 error_log)**: Session pooler の上限 15 本を API 2 台 × 30 本が超過 (EMAXCONNSESSION) = GAP-291。transaction pooler へ切替後に再計測 | T-I-24 | L4 |
| PS-30 | DB 接続の上限 (G-11) | API が使う接続文字列の種類 (直結 / Session pooler / Transaction pooler) と、その上限本数を実環境で確認し、`ATELIER_DB_POOL_SIZE + MAX_OVERFLOW × 台数` が上限を超えていないことを見る | API の要求本数 ≤ 接続先の上限。Session pooler (15 本) を複数台で共有しない。transaction pooler なら prepared statement 無効 (自動) と押し出し通知の直結 (`ATELIER_DB_LISTEN_URL`) が設定済み | 未計測 (切替後に実施) | T-I-24 | L4 |
