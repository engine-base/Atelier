...
構成
----
  フロー一覧   23 フローを 1 行ずつ。進捗と状態が一目で分かる（**最初に開くシート**）
...
def build_overview(wb: Workbook, rows: list[dict]) -> None:
    """最初に開くシート。**23 フローを 1 行ずつ**。ここだけ見れば全体が分かる。"""
...
def build_roles(wb: Workbook, plan: dict) -> None:
    ...
    style_header(ws, 1, 5)
    set_widths(ws, [16, 30, 32, 34, 30])
    for role, entry, goal, prov, cons in [
        (
            "運営 (admin)",
            "通常サインイン + 運営権限",
            "全アカウントの土台を用意する",
            "既定テンプレ / AI 社員テンプレ / スキル / 法務文書",
            "監査ログ / 健全性 / 混雑実績",
        ),
        (
            "オーナー (owner)",
        ...
        (
            "クライアント (client_portal)",
            "招待リンク (別 cookie 系統)",
            "進捗を見て意見を返す",
            "コメント",
            "限定ビュー (自分の案件のみ)",
        ),
        ("未サインイン (guest)", "—", "公開ページを読む", "—", "規約 / プライバシー / 特商法"),
    ]:
        ws.append([role, entry, goal, prov, cons])
        for c in range(1, 6):
    ...
    ws.append([])
    ws.append(["実行順の原則", plan["discovered"]["dependency_note"]])
