...
describe('ProcessContextBar (T-UC-09)', () => {
  it('marks current phase with aria-current', () => {
    render(
      <ProcessContextBar phases={['A', 'B']} currentPhaseId="B" onChange={() => undefined} />,
    );
  ...
  it('invokes onChange on click', () => {
    ...
    render(<ProcessContextBar phases={['A', 'B']} currentPhaseId="A" onChange={onChange} />);
...
describe('OutputViewer (T-UC-12)', () => {
  const pins: CommentPin[] = [{ id: 'p1', x: 10, y: 10, text: 'x', author: 'wanda' }];

  it('renders title, content, and pin buttons', () => {
    render(<OutputViewer title="T" contentHtml="<p>body</p>" pins={pins} />);
    expect(screen.getByRole('heading', { name: 'T' })).toBeInTheDocument();
    expect(screen.getByText('body')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /コメント by wanda/ })).toBeInTheDocument();
  });

  it('renders comment list', () => {
    render(<OutputViewer title="T" contentHtml="" pins={pins} />);
    expect(screen.getByRole('list', { name: 'コメント一覧' })).toBeInTheDocument();
  });
});
...
describe('MockViewer (T-UC-13)', () => {
  it('renders title and viewport toggles', () => {
    ...
    expect(screen.getByRole('group', { name: 'ビューポート切替' })).toBeInTheDocument();
  });
...
describe('TranscriptUpload (T-UC-23)', () => {
  ...
  it('shows error message on upload failure', async () => {
    ...
    const file = new File(['x'], 'a.wav', { type: 'audio/wav' });
    await act(async () => {
      fireEvent.change(screen.getByLabelText(/音声/), { target: { files: [file] } });
      await new Promise((r) => setTimeout(r, 0));
    });
...
describe('SalesDocDraft (T-UC-24)', () => {
  ...
  it('generates and displays draft on submit', async () => {
    ...
    render(<SalesDocDraft onDraft={onDraft} />);
    fireEvent.change(screen.getByLabelText(/顧客名/), { target: { value: 'ACME' } });
    fireEvent.change(screen.getByLabelText(/案件/), { target: { value: 'X 検討' } });
