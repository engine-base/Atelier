...
test("S-A03: 初期値 (WS名) + AI学習は既定OFF", async ({ page, context }) => {
  ...
  await page.goto(`/auth/s_a03?workspace=${IDS.ws}`, { waitUntil: "networkidle" });
...
test("S-C02: 表示名空 → 入力必須 / 保存 422 → alert", async ({ page, context }) => {
  await signin(context);
  await page.goto(`/employees/s_c02?employee=${IDS.employee}`, {
...
test("S-L01: 招待発行 → 一覧反映、失効 → 状態変化", async ({ page, context }) => {
  await signin(context);
  await page.goto(`/client/s_l01?project=${IDS.project}`, {
    waitUntil: "networkidle",
  });
...
test("S-L03: loading / 空 / 403 / 5xx (client cookie)", async ({
  ...
  await page.goto(`/client/s_l03?project=${IDS.project}`, { waitUntil: "commit" });
  await expect(
  ...
  await page.goto(`/client/s_l03?project=${IDS.project}`, { waitUntil: "networkidle" });
...
test("S-M01: ファイル選択 input が実在し操作可能", async ({ page, context }) => {
  await signin(context);
  await page.goto(`/upload/s_m01?project=${IDS.project}`, {
    waitUntil: "networkidle",
  });
