...
> **現在の運用（2026-09-03〜ローンチ前）**: Supabase Free の上限で別プロジェクトを作れないため、**本番を staging として共用**している
> （ADR-021 改訂）。この手順書は **ローンチ前の切替時**（Pro 化後）に使う。それまで L1〜L3 は本番で QA 専用データにだけ触って流し、
> 結果列に `prod(pre-launch)` と書く。
