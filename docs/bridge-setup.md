...
| 実行モードの検証 | サーバーが送ってきた `tools_mode` を既知の 3 値に正規化する（想定外の文字列で強い権限に倒れない）。**上限は掛けない** — Claude Code と同じ挙動のまま |
...
**正直に書いておくこと**: auto モードは「確認なしで bash を動かす」という約束で、
これは Claude Code と同じ考え方（経営者判断 2026-08-19「承認モードや bash は
Claude もやってるしそのままでいい」）。ここで塞いだのは**指示元のすり替え**であって、
auto そのものは変えていない。接続トークンは今も平文 JSON (mode 0600) で、
OS キーチェーンへの移行は未実施。
