...
## まず誤解を解く: ローテーションは「中身」を変えない・勝手に走らない

- **保管したシークレットの値 (Supabase キー / API キー / パスワード等) は
  1 文字も変わらない。** ローテーションが交換するのは「保管庫の扉の鍵
  (Atelier がデータを暗号化するための鍵)」だけ。中身は復号 → 新しい鍵で
  包み直されるのみで、reveal で取り出す値は前後で完全に同一 (e2e で
  同一値を突合済)。預け先 (Supabase 等) 側の鍵には一切触れない。
- **自動では絶対に実行されない。** スケジューラ・CI・AI からの呼び出し経路は
  存在せず、経営者が `rotate_vault_key.py` を手で実行したときだけ動く。
  さらに実行時は確認入力 (`rotate` と打つ) が必須 (`--yes` で明示スキップ)。
- ではなぜローテーション「手段」が要るのか: 鍵が漏れた疑いがある時・
  委託先の離任時に、**中身を再登録せずに**扉の鍵だけ替えるため。手段が無いと
  その時に全シークレットの再発行 (顧客巻き込み) になる。
