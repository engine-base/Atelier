...
2. **独立検証を実際に実行する** (dev のログの読み直しは検証ではない)。
   検証は **`human-grade-qa` スキル (Diff モード = 今回の変更範囲) を起動して**
   その手順で行い、テスト実行の整合確認には `test-verification` スキル、
   PM からマイルストーン回帰の指示があった場合は **`e2e-journey-walkthrough`
   スキルで通し**を実施する。スキルが起動できない環境なら、その旨を QA レポートに
   明記した上で以下を手動で全数実施する:
...
## A/B 検証 (修正前後の比較) の手順 — 必須

`docs/agents/protocol.md`「検証用 worktree」に従う。要点:

1. **共有ツリーで測らない。** `git worktree add` で隔離したツリーを作り、そこで
   install → build → 測定する。共有ツリーのファイルを一時差し替えする運用は禁止。
2. **ビルド直前に対象ファイルの中身を自分で表示・記録**する。
3. **成果物のチャンク名/ハッシュを記録**し、どのビルドの産物かを対応づける。
4. **自分の測定が他者の申告と食い違ったら、正しさを論じる前に「両者が実際に何を測ったか」の
   射程を確認する。** 特に「相手は修正前を、自分は修正後を測っていないか」を先に疑う。
5. 「修正前は壊れていなかった」は、**修正前を自分で再構成して測ったときにのみ**言う。
   観測が自分の後の測定と合わないというだけで、先の観測を「残骸」と切り捨てない。

> 束 C でこの 5 点すべてに反する誤りが実際に起き、dev の正しい報告を否定してしまった。
> 検証水準そのものは高くても、**測定対象を取り違えれば結論は反転する**。
