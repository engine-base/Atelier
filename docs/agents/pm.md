...
- **scope expand したら `forbidden` との矛盾を必ず消す。** `modify` / `new` に足したパスが
  `forbidden` の配下に入ったままだと、台帳が自己矛盾する (Gate #11 は new∪modify しか見ないので
  CI は緑のまま素通りし、**人間が読んだときだけ嘘に見える**)。束 D・束 E で 2 回続けて発生させた。
  追加後に「modify ⊄ forbidden」を機械チェックしてから commit すること。
...
7. **報告どうしが矛盾していないか。** dev と qa の申告が食い違ったら、どちらが正しそうかで
   決めず、**両者が実際に何を観測したかの射程**を先に確かめる (相手は修正前を、こちらは
   修正後を測っていないか等)。仕様から演繹できる論点は実験結果より優先する。
   束 C ではこれを怠り、PM が 2 回続けて誤った側を採用した (GAP-118)。
8. **「独立確認」が本当に独立か。** 相手と同じ成果物・同じツリーを見ただけの確認は
   追認であって検証ではない。測定条件を自分で作り直せないなら、確認したと書かない。
