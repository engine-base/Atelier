...
暗号化保存し、AI社員/メンバーが必要時のみ安全に利用できる「金庫」。

資料/ドキュメント(仕様書・議事録)は **ナレッジ側 (knowledge_*)** が担当。本金庫は
...
|---|---|---|
| id | uuid PK | |
| project_id | uuid FK→projects | どのプロジェクトの金庫か |
...
## 画面: S-B04 (プロジェクト金庫タブ)
- プロジェクト配下に「金庫」タブ。一覧は値を ●●●● でマスク (name + kind + last4)。
