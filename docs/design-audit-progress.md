...
## 残り 8 画面 (全画面に仕様書あり — v2 監査の続行のみ)

- **次:** S-L03 クライアントポータル → T-UC-36〜40 ほか (残 8)
- 仕様書ありで未監査: S-L03/T-UC-36〜40 ほか

## 未解消 gap (正本: docs/gap-tracker.md)

- 機能 gap: GAP-001〜028 (28 件) — バックエンド API 不在により UI から撤去/未描画にしたもの。**GAP-016 (Whisper worker 不在) は S-M01 の主機能を塞ぐ最重要 gap**
