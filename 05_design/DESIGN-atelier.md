...
### Progress Color Ramp — 進捗ゲージの色ランプ

進捗管理系のゲージ（プロジェクトカードの 9 段工程バー / 全体進捗 / タスク完了% / AC 達成率 / 実行スコア）は、
**1 本のゲージ内にグラデーションをかけない。塗りは常に単色**で、進捗が進むほどランプ
`#14B8A6 (teal) → #84CC16 (lime) → #F59E0B (amber)` 上の色に進む（2026-08-17 経営者指示の確定形:
「単色で、工程が進むごとに色がグラデーション的になる。ゲージ内グラデーションと動きは不要」）。
実体は design-tokens の `progressRamp` と補間関数 `progressColor(t)`。アニメーション付与・ゲージ以外への流用は不可。
例外: 「要対応」状態のゲージは semantic に secondary 単色を維持する。
