...
### Progress Gauge Gradient — 進捗ゲージ専用グラデーション

進捗管理系のゲージ（プロジェクトカードの 9 段工程バー / 全体進捗 / タスク完了% / AC 達成率 / 実行スコア / アップロード進行）は、
ブランド単色ではなく**鮮やかなグラデーション** `linear-gradient(90deg, #14B8A6 → #84CC16 → #F59E0B)` を使用する
（2026-08-17 経営者指示: 「進捗管理のゲージのカラーは変えなくていい。グラデーションっぽく鮮やかに」）。
実体は design-tokens の `gradients.progress`（Tailwind クラス `bg-progress-gradient`）。ゲージ以外への流用は不可。
